from fastapi import APIRouter, Depends, HTTPException,status,Query,UploadFile,Form,File,Request
...
application_router = APIRouter(prefix="/files", tags=["files"])
...
# ---------------------------------------------------------------------
# CREATE APPLICATION
# Only users with roles "TP" or "Non TP" can apply
...
async def create_application(
    ...
    # 3. Build application doc (just the IDs here)
    application_data = {
        "_id": str(uuid.uuid4()),
        "job_rr_id": job_rr_id,
        "employee_id": str(current_user["employee_id"]),
...
@application_router.post("/{app_id}/submit")
